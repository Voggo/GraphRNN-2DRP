...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        x[0, 1:, :] = (self.data_bfs_adj[index])
        y[0, :, :] = (self.data_bfs_adj[index])
        x[1, 1:, :] = (edge_dir[:, :, 0])
        y[1, :, :] = (edge_dir[:, :, 0])
        x[2, 1:, :] = (edge_dir[:, :, 1])
        y[2, :, :] = (edge_dir[:, :, 1])
        ...
        x[6, 1:, :] = (self.data_bfs_offset[index])
        y[6, :, :] = (self.data_bfs_offset[index])
        x[7, 1:, :] = (self.data_nodes_width[index].T)
        x[8, 1:, :] = (self.data_nodes_height[index].T)
        x[9, 1:, :] = (self.data_nodes_width[index])
        x[10, 1:, :] = (self.data_nodes_height[index])
        y = torch.tensor(y)
        x = torch.tensor(x)
        return {"x": x, "y": y}
