...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        x = np.zeros((10, self.max_num_nodes, self.max_num_nodes))
        y = np.zeros((6, self.max_num_nodes - 1, self.max_num_nodes))
        edge_dir = np.array(F.one_hot(self.data_bfs_edge_dir[index], 4))
        x[:, 0:1, :] = np.ones((10, 1, self.max_num_nodes))
        ...
        x[5, 1:, :] = np.tril(self.data_bfs_offset[index][1:, :])
        y[5, :, :] = np.tril(self.data_bfs_offset[index][1:, :])
        x[6, :, :] = np.tril(self.data_nodes_width[index].T)
        x[7, :, :] = np.tril(self.data_nodes_height[index].T)
        x[8, :, :] = np.tril(self.data_nodes_width[index])
        x[9, :, :] = np.tril(self.data_nodes_height[index])
        y = torch.tensor(y)
        x = torch.tensor(x)
        return {"x": x, "y": y}
